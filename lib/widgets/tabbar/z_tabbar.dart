...
// 源文章： https://blog.csdn.net/zhumj_zhumj/article/details/102832360
// https://github.com/zhumj/flutter_ZTabBar/blob/master/lib/controller/HomePage.dart
///Add by zhumj
class ZTabHelper {
  const ZTabHelper({
    this.text,
...
class ZTab extends StatelessWidget {
  ...
  const ZTab({
    ...
    this.textToIconMargin = 0.0,
        ...
        assert(!(text != null &&
            null != child)), // https://github.com/dart-lang/sdk/issues/34180
        super(key: key);

  ///Add by zhumj
  /// Tab高度，默认 _kTabHeight
  /// Tab height, default [_kTabHeight]
  final double tabHeight;

  /// Tab 文字与图标的距离，默认 0.0
  /// Tab text and icon distance, default 0.0
  ...
  Widget build(BuildContext context) {
    ...
    ///The custom height is used if the Tab custom height is greater than _kTabHeight
...
class _ZTabStyle extends AnimatedWidget {
  const _ZTabStyle({
    Key key,
...
typedef _ZLayoutCallback = void Function(
    List<double> xOffsets, TextDirection textDirection, double width);

class _ZTabLabelBarRenderer extends RenderFlex {
  _ZTabLabelBarRenderer({
  ...
  _ZLayoutCallback onPerformLayout;
...
class _ZTabLabelBar extends Flex {
  _ZTabLabelBar({
    Key key,
  ...
  final _ZLayoutCallback onPerformLayout;

  @override
  RenderFlex createRenderObject(BuildContext context) {
    return _ZTabLabelBarRenderer(
      direction: direction,
      mainAxisAlignment: mainAxisAlignment,
      mainAxisSize: mainAxisSize,
      crossAxisAlignment: crossAxisAlignment,
  ...
  void updateRenderObject(
      BuildContext context, _ZTabLabelBarRenderer renderObject) {
...
class _ZIndicatorPainter extends CustomPainter {
  _ZIndicatorPainter({
    @required this.controller,
    @required this.indicator,
    @required this.indicatorSize,
    @required this.tabKeys,
    _ZIndicatorPainter old,
  ...
  @override
  bool shouldRepaint(_ZIndicatorPainter old) {
    return _needsPaint ||
...
class _ZChangeAnimation extends Animation<double>
    with AnimationWithParentMixin<double> {
  _ZChangeAnimation(this.controller);
...
class _ZDragAnimation extends Animation<double>
    with AnimationWithParentMixin<double> {
  _ZDragAnimation(this.controller, this.index);
...
class _ZTabBarScrollPosition extends ScrollPositionWithSingleContext {
  _ZTabBarScrollPosition({
...
class _ZTabBarScrollController extends ScrollController {
  _ZTabBarScrollController(this.tabBar);
...
class ZTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// is [ZTabBarIndicatorSize.tab] the tab's bounds are as wide as the space
  /// occupied by the tab in the tab bar. If [indicatorSize] is
  /// [ZTabBarIndicatorSize.label], then the tab's bounds are only as wide as
  ...
  /// tab's overall bounds if [indicatorSize] is [ZTabBarIndicatorSize.tab]
  /// (the default) or relative to the bounds of the tab's widget if
  /// [indicatorSize] is [ZTabBarIndicatorSize.label].
  ///
...
class _ZTabBarState extends State<ZTabBar> {
  ...
  _ZIndicatorPainter _indicatorPainter;
  int _currentIndex;
  ...
  void initState() {
    ...
    widget.tabs.forEach((zTabHelper) {
      _tabs.add(ZTab(
        text: zTabHelper.text,
        icon: zTabHelper.icon,
        child: zTabHelper.child,
        tabHeight: widget.tabHeight,
        textToIconMargin: _textToIconMargin,
      ));
    });
  ...
  Widget build(BuildContext context) {
    ...
    if (_controller != null) {
      ...
      if (_controller.indexIsChanging) {
        ...
        final Animation<double> animation = _ZChangeAnimation(_controller);
        ...
        final Animation<double> centerAnimation =
            _ZDragAnimation(_controller, tabIndex);
        ...
        if (_currentIndex > 0) {
          ...
          final Animation<double> previousAnimation =
              ReverseAnimation(_ZDragAnimation(_controller, tabIndex));
          wrappedTabs[tabIndex] =
    ...
    if (widget.isScrollable) {
      _scrollController ??= _ZTabBarScrollController(this);
